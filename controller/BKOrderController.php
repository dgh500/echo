...
class OrderController {
	...
	function __construct() {
		...
		$this->mVendor = $registry->paymentVendor;
	...
	function CreateOrder($basket) {
		if ($this->OrderAlreadyExistsWithBasket($basket)) {
	...
	//! Creates an order and returns it as Obj:OrderModel
	/*!
	 * @param [in] $googleOrderId - Google's own order ID, used as a basket ID with google orders
	 * @return Obj:OrderModel - the order created
	 */
	function CreateGoogleOrder($googleOrderId) {
		if ($this->GoogleOrderAlreadyExists($googleOrderId)) {
			return $this->GetGoogleOrder($googleOrderId);
		}
		$orderStatusController 	= new OrderStatusController();
		$currencyController 	= new CurrencyController();
		$courierController 		= new CourierController();
		...
		INSERT INTO tblOrder
			(`Created_Date`,`Basket_ID`,`Status_ID`,`Notes`,`Currency_ID`,`Downloaded`,`Brochure`,`Courier_ID`,`Dispatch_Date_ID`,`Postage_Method_ID`)
			VALUES
			(\''.time().'\',\''.$googleOrderId.'\',\''.$orderStatusController->GetDefault()->GetStatusId().'\',\'\',\''.$currencyController->GetDefault()->GetCurrencyId().'\',\'0\',\'0\',\''.$courierController->GetDefault()->GetCourierId().'\',\''.$dispatchDateController->GetDefaultDispatchDate()->GetDispatchDateId().'\',\''.$postageMethodController->GetDefault()->GetPostageMethodId().'\')
		';
		if ($this->mDatabase->query($create_order_sql)) {
			$sql = 'SELECT Order_ID FROM tblOrder ORDER BY Order_ID DESC LIMIT 1';
			if ($result = $this->mDatabase->query ( $sql )) {
	...
	} // End CreateGoogleOrder

	function GetGoogleOrder($googleOrderId) {
		$sql = 'SELECT Order_ID FROM tblOrder WHERE Basket_ID = \''.$googleOrderId.'\'';
		$result = $this->mDatabase->query($sql);
		$resultObj = $result->fetchObject();
		return new OrderModel($resultObj->Order_ID);
	}

	//! Returns whether or not an order already exists with the basket supplied
	/*
	 * @param $basket - Obj : BasketModel - The basket to check against
	 * @return Boolean - True if an order DOES alrady exist with that basket
	 */
	function OrderAlreadyExistsWithBasket($basket) {
		$sql = 'SELECT COUNT(Order_ID) AS OrderCount FROM tblOrder WHERE Basket_ID = \'' . $basket->GetBasketId () . '\'';
		$result = $this->mDatabase->query ( $sql );
		$resultObj = $result->fetchObject ();
	...
	} // End OrderAlreadyExistsWithBasket

	//! Returns whether or not an order already exists with the google order number
	/*
	 * @param $googleOrderNumber - Str - The Google order number
	 * @return Boolean - True if an order DOES alrady exist with that basket
	 */
	function GoogleOrderAlreadyExists($googleOrderNumber) {
		$sql = 'SELECT COUNT(Order_ID) AS OrderCount FROM tblOrder WHERE Basket_ID = \''.$googleOrderNumber.'\'';
		$result = $this->mDatabase->query($sql);
		$resultObj = $result->fetchObject();
		if ($resultObj->OrderCount > 0) {
			return true;
	 ...
	 * @return Either Bool False on failure, or the result (raw)
	 */
	function SendPaypalOrderRequest($request, $requestUrl) {
		$ch = curl_init ();
		// Set the URL
		curl_setopt ( $ch, CURLOPT_URL, $requestUrl );
		// No headers
	...
	// Takes a NVP string and returns a key/value array
	function FormatPaypalResponse($rawResponse) {
		return parse_url($rawResponse);
	}
	 ...
	 * @param $ThreeDSecure - Int - Fine tune 3D Secure rules
	 *		0 = If enabled, use them (in admin panel)
	 ...
	 * @param $accountType - The account type to use for the request, E,C or M. Defaults to E(-Commerce) for web transactions, set to M for phone orders
	 *		E = E-Commerce
	 *		C = Continuous (NA)
	 *		M = MOTO - Mail Order Telephone Order
	 * @return Str - The XML to send
	 */
	function ConstructOrderRequest($order, $billingDetails, $prefix = 'ECHO', $avsCheck = 0, $ThreeDSecure = 0, $accountType = 'E') {
		// Validate
		...
		$BILLING_ADDRESS1  		= substr(trim($validationHelper->OnlyAlphaNumeric($billingAddress->GetLine1())),0,98);					// Max 100 Chars
		$BILLING_ADDRESS2 		= substr(trim($validationHelper->OnlyAlphaNumeric($billingAddress->GetLine2())),0,98);					// Max 100 Chars
		$BILLING_ADDRESS_CITY 	= substr(trim($validationHelper->OnlyAlphaNumeric($billingAddress->GetLine2())),0,38);					// Max 40 Chars
		$BILLING_POSTCODE 		= substr(trim($validationHelper->OnlyAlphaNumeric($billingAddress->GetPostcode())),0,9);				// Max 10 Chars
		$BILLING_COUNTRY		= substr(trim($validationHelper->OnlyAlphaNumeric($billingAddress->GetCountry()->GetTwoLetter())),0,2);	// Max 2 Chars
		if($BILLING_ADDRESS1=='') 		{ $BILLING_ADDRESS1 	= 'LINE1'; }
		if($BILLING_ADDRESS2=='') 		{ $BILLING_ADDRESS2 	= 'LINE2'; }
		if($BILLING_ADDRESS_CITY=='') 	{ $BILLING_ADDRESS_CITY= 'CITY'; }
		if($BILLING_POSTCODE=='') 		{ $BILLING_POSTCODE 	= 'POSTCODE'; }
		if($BILLING_COUNTRY=='') 		{ $BILLING_COUNTRY 	= 'XX'; }

		// Billing Customer Details
		$BILLING_FIRST_NAMES  	= substr(trim($order->GetCustomer()->GetFirstName()),0,18);	// Max 20 Chars
		$BILLING_SURNAME  	  	= substr(trim($order->GetCustomer()->GetFirstName()),0,18); // Max 20 Chars

		// Get Delivery Address
		$deliveryAddress = $order->GetShippingAddress();
		$DELIVERY_ADDRESS1  	= substr(trim($validationHelper->OnlyAlphaNumeric($deliveryAddress->GetLine1())),0,98);						// Max 100 Chars
		$DELIVERY_ADDRESS2 		= substr(trim($validationHelper->OnlyAlphaNumeric($deliveryAddress->GetLine2())),0,98);						// Max 100 Chars
		$DELIVERY_ADDRESS_CITY 	= substr(trim($validationHelper->OnlyAlphaNumeric($deliveryAddress->GetLine2())),0,38);						// Max 40 Chars
		$DELIVERY_POSTCODE 		= substr(trim($validationHelper->OnlyAlphaNumeric($deliveryAddress->GetPostcode())),0,9);					// Max 10 Chars
		$DELIVERY_COUNTRY		= substr(trim($validationHelper->OnlyAlphaNumeric($deliveryAddress->GetCountry()->GetTwoLetter())),0,2);	// Max 2 Chars
		if($DELIVERY_ADDRESS1=='') 		{ $DELIVERY_ADDRESS1 	= 'LINE1'; }
		if($DELIVERY_ADDRESS2=='') 		{ $DELIVERY_ADDRESS2 	= 'LINE2'; }
		if($DELIVERY_ADDRESS_CITY=='') 	{ $DELIVERY_ADDRESS_CITY= 'CITY'; }
		if($DELIVERY_POSTCODE=='') 		{ $DELIVERY_POSTCODE 	= 'POSTCODE'; }
		if($DELIVERY_COUNTRY=='') 		{ $DELIVERY_COUNTRY 	= 'XX'; }

		// Billing Customer Details
		$DELIVERY_FIRST_NAMES  	= substr(trim($order->GetCustomer()->GetFirstName()),0,18);	// Max 20 Chars
		$DELIVERY_SURNAME  	  	= substr(trim($order->GetCustomer()->GetFirstName()),0,18); // Max 20 Chars

		// Contact Phone Number (For Fraud)
		$CONTACT_NUMBER = $order->GetCustomer()->GetDaytimeTelephone();
		...
		$request .= '&TxType=DEFERRED';
		$request .= '&Vendor=' . $this->mVendor;
		...
		$request .= '&BillingSurname='.$BILLING_SURNAME;
		$request .= '&BillingFirstnames='.$BILLING_FIRST_NAMES;
		$request .= '&BillingAddress1='.$BILLING_ADDRESS1;
		$request .= '&BillingAddress2='.$BILLING_ADDRESS2;
		$request .= '&BillingCity='.$BILLING_ADDRESS_CITY;
		$request .= '&BillingPostCode='.$BILLING_POSTCODE;
		$request .= '&BillingCountry='.$BILLING_COUNTRY;
		$request .= '&DeliverySurname='.$DELIVERY_SURNAME;
		$request .= '&DeliveryFirstnames='.$DELIVERY_FIRST_NAMES;
		$request .= '&DeliveryAddress1='.$DELIVERY_ADDRESS1;
		$request .= '&DeliveryAddress2='.$DELIVERY_ADDRESS2;
		$request .= '&DeliveryCity='.$DELIVERY_ADDRESS_CITY;
		$request .= '&DeliveryPostCode='.$DELIVERY_POSTCODE;
		$request .= '&DeliveryCountry='.$DELIVERY_COUNTRY;
		$request .= '&ContactNumber='.$CONTACT_NUMBER;
	 ...
	 * @param $billingDetails : Array - The card numbers etc. in format:
	 *		['cardNumber'],['expiryDateMonth'],['expiryDateYear'],['cvn'],['cardHoldersName'],['validFromMonth'],['validFromYear'],['issueNumber']
	 * @param $prefix : Str - Either 'ECH0' or 'ECHL0' for production/testing purposes
	 */
	function ConstructPaypalOrderRequest($order, $billingDetails, $prefix = 'ECH0') {
		// Validate
		$validationHelper = new ValidationHelper;
		$registry = Registry::getInstance();
		...
		$CARD_NUMBER 	= $billingDetails['cardNumber'];
		$CARD_TYPE 		= $this->CardType($CARD_NUMBER);
		$EXPIRY_DATE 	= $billingDetails['expiryDateMonth'].$billingDetails['expiryDateYear']; // MMYYYY Format
		$CVV_NUMBER 	= $billingDetails['cvn'];
		$CARD_HOLDERS_NAME = $billingDetails ['cardHoldersName'];
		$ORDER_TOTAL 	= trim(number_format($order->GetTotalPrice()+$order->GetTotalPostage(),2)); // 2 Decimal Places
		$ORDER_POSTAGE 	= trim(number_format($order->GetTotalPostage(),2)); 						// 2 Decimal Places
		...
		// Billing Customer Details
		$BILLING_FIRST_NAMES  	= substr(trim($order->GetCustomer()->GetFirstName()),0,18);	// Max 20 Chars
		$BILLING_SURNAME  	  	= substr(trim($order->GetCustomer()->GetLastName()),0,18); // Max 20 Chars
		...
		// Billing Customer Details
		$DELIVERY_FIRST_NAMES  	= substr(trim($order->GetCustomer()->GetFirstName()),0,18);	// Max 20 Chars
		$DELIVERY_SURNAME  	  	= substr(trim($order->GetCustomer()->GetLastName()),0,18); // Max 20 Chars
		...
		// Construct Request
		$request = 'VERSION='.urlencode('56.0');
		$request .= '&SIGNATURE='.urlencode($registry->paypalApiSignature);
		$request .= '&USER='.urlencode($registry->paypalApiUsername);
		$request .= '&PWD='.urlencode($registry->paypalApiPassword);
		$request .= '&METHOD='.urlencode('DoDirectPayment');
		$request .= '&AMT='.$ORDER_TOTAL;
		$request .= '&CURRENCYCODE='.urlencode('GBP');
		$request .= '&PAYMENTACTION='.urlencode('Sale');
		$request .= '&IPADDRESS='.urlencode($_SERVER['REMOTE_ADDR']);
		$request .= '&CREDITCARDTYPE='.$CARD_TYPE;
		$request .= '&ACCT='.$CARD_NUMBER;
		$request .= '&EXPDATE='.$EXPIRY_DATE;
		$request .= '&CVV2=' . $CVV_NUMBER;
		$request .= '&FIRSTNAME='.$BILLING_FIRST_NAMES;
		$request .= '&LASTNAME='.$BILLING_SURNAME;
		$request .= '&STREET='.$BILLING_ADDRESS1;
		$request .= '&CITY='.$BILLING_ADDRESS_CITY;
		$request .= '&STATE='.$BILLING_ADDRESS2;
		$request .= '&COUNTRYCODE='.$BILLING_COUNTRY;
		$request .= '&ZIP='.$BILLING_POSTCODE;
		$request .= '&SHIPTONAME='.$DELIVERY_FIRST_NAMES.$DELIVERY_SURNAME;
		$request .= '&SHIPTOSTREET='.$DELIVERY_ADDRESS1;
		$request .= '&SHIPTOCITY='.$DELIVERY_ADDRESS_CITY;
		$request .= '&SHIPTOSTATE='.$DELIVERY_ADDRESS2;
		$request .= '&SHIPTOZIP='.$DELIVERY_POSTCODE;
		$request .= '&SHIPTOCOUNTRY='.$DELIVERY_COUNTRY;

		#die($request);
		return $request;
	} // End ConstructPaypalOrderRequest
	...
	function ConstructShipRequest($order, $shipTotal, $prefix = 'ECHO') {
		...
		// Initialise Values
		$AMOUNT = str_replace ( ',', '', trim ( number_format ( $shipTotal, 2 ) ) );
		$ORDER_ID = $prefix . $order->GetOrderId ();
		$TRANSACTION_ID = $order->GetTransactionId ();
		$SECURITY_KEY = $order->GetSecurityKey ();
		$AUTH_REFERENCE = $order->GetTxAuthNo ();
		...
		$request .= '&TxAuthNo=' . $AUTH_REFERENCE;
		$request .= '&ReleaseAmount=' . $AMOUNT;
		return $request;
	...
	function ConstructCancelRequest($order, $prefix = 'ECHO') {
		...
		$TRANSACTION_ID = $order->GetTransactionId ();
		$SECURITY_KEY = $order->GetSecurityKey ();
		$TX_AUTH_NO = $order->GetTxAuthNo ();

		// Construct Request
		$request = 'VPSProtocol=2.23';
		$request .= '&TxType=ABORT';
		$request .= '&Vendor=' . $this->mVendor;
		$request .= '&VendorTxCode=' . $ORDER_ID;
		$request .= '&VPSTxId=' . $TRANSACTION_ID;
		$request .= '&SecurityKey=' . $SECURITY_KEY;
		$request .= '&TxAuthNo=' . $TX_AUTH_NO;
		return $request;
	} // End ConstructShipRequest
	...
	function PrepareAndSendCustomerEmail($address, $order, $prefix = 'ECHO') {
		$registry = Registry::getInstance ();
		$orderView = new PublicOrderView ( );

		$body = 'Your order: ' . $prefix . $order->GetOrderId () . ' has been received. Your bank has reserved the payment for this order, and no money is taken until goods are dispatched. If there is any delay with your order you will receive an email with an estimated delivery time. Once your order has been dispatched you will receive a final email detailing goods that have been shipped and if applicable a tracking link and number with which you can track the status of your delivery.<br />';
		$body .= $orderView->LoadDefault ( $order->GetOrderId () );
		$text_body = 'Order '.$prefix.$order->GetOrderId () . ' received. Your bank has reserved the payment for this order, and no money is taken until goods are dispatched. If there is any delay with your order you will receive an email with an estimated delivery time. Once your order has been dispatched you will receive a final email detailing goods that have been shipped and if applicable a tracking link and number with which you can track the status of your delivery. \r\n';
	...
	} // End PrepareAndSendCustomerEmail

	function PrepareAndSendEmail($address, $order, $prefix = 'ECHO') {
		$registry = Registry::getInstance ();
		$orderView = new InternalOrderView ( );
		$body = $orderView->LoadDefault ( $order->GetOrderId () );
		$this->mMail = new PHPMailer;
		$this->mMail->From = "info@echosupplements.com";
		$this->mMail->FromName = "Website Order - ".$prefix.$order->GetOrderId();
	...
	function PrepareAndSendFailureEmail($address, $order, $prefix = 'ECHO') {
		$registry = Registry::getInstance ();
		$orderView = new InternalOrderView ( );
		$body = $orderView->LoadDefault ( $order->GetOrderId () );
		...
		$this->mMail = new PHPMailer;
		$this->mMail->From = "info@echosupplements.com";
		$this->mMail->FromName = "Website Order - ".$prefix.$order->GetOrderId().' - FAILED';
		$this->mMail->Subject = "Website Order - ".$prefix.$order->GetOrderId().' - FAILED';
		$this->mMail->Host = "smtp.gmail.com";
		$this->mMail->Port = 465;
		$this->mMail->Mailer = "smtp";
		$this->mMail->SMTPAuth = true;
	...
	function CardType($cardNumber) {
		...
		switch ($firstTwoNumbers) {
			case in_array ( $firstTwoNumbers, $masterCard ) :
				return 'MC';	// MC for SagePay, MASTERCARD for Paypal
				break;
			case in_array ( $firstTwoNumbers, $solo ) :
				return 'SOLO';
				break;
		}
	...
	//! Gets all orders placed within the date range for the catalogues supplied
	/*!
	 ...
	 * @return $retArr			- OrderModel - An array of OrderModel orders
	 */
	function GetOrders($catalogueList, $startTimestamp = false, $endTimestamp = false) {
		// Set Defaults
		if (! $startTimestamp) {
		...
		/*$sql = 'SELECT DISTINCT Order_ID
				...
				WHERE Created_Date BETWEEN ' . $startTimestamp . ' AND ' . $endTimestamp . ' AND tblCategory.Catalogue_ID IN (' . $catalogueList . ') AND tblOrder.Status_ID IN (3,6,7,10)
				ORDER BY Order_ID DESC';*/
		$sql = 'SELECT DISTINCT Order_ID
				FROM tblOrder
				WHERE Created_Date BETWEEN ' . $startTimestamp . ' AND ' . $endTimestamp . '
				AND tblOrder.Catalogue_ID IN (' . $catalogueList . ')
				AND tblOrder.Status_ID IN (3,6,7,10)
				ORDER BY Order_ID DESC';
		if ($result = $this->mDatabase->query ( $sql )) {
			$orders = $result->fetchAll ( PDO::FETCH_OBJ );
			// For each order, create a new instance of it and store it in the todaysOrders variable
			...
			$error = new Error ( 'Could not get orders.' );
			$error->PdoErrorHelper ( $this->mDatabase->errorInfo (), __LINE__, __FILE__ );
			throw new Exception ( $error->GetErrorMsg () );
		}
	} // End GetTodaysOrders
	 ...
	 * @param $endTimestamp		- Int - The timestamp to constrain the end of the search to, defaults to current time
	 */
	function GetReferrers($catalogueList, $startTimestamp = false, $endTimestamp = false) {
		// Set Defaults
		if (! $startTimestamp) {
			$startTimestamp = mktime ( 0, 0, 0, 1, 1, 2008 );
		}
		...
		$sql = 'SELECT	COUNT(tblOrder.Referrer_ID) AS ReferrerCount,
							SUM(tblOrder.Total_Actually_Taken) AS TotalSpend,
							tblOrder.Referrer_ID
					FROM tblOrder
					WHERE tblOrder.Created_Date BETWEEN '.$startTimestamp.' AND '.$endTimestamp.' AND tblOrder.Catalogue_ID IN ('.$catalogueList.')
					AND tblOrder.Status_ID IN (3,6,7,10) AND tblOrder.Referrer_ID IS NOT NULL GROUP BY tblOrder.Referrer_ID
		...
		if ($result = $this->mDatabase->query ( $sql )) {
			...
			foreach ( $referrerResult as $resultObj ) {
					// Add to array
					$nDaysReferrers[$resultObj->Referrer_ID]['referrerCount'] = $resultObj->ReferrerCount;
					$nDaysReferrers [$resultObj->Referrer_ID]['totalSpend'] = $resultObj->TotalSpend;
			}
			...
			$error = new Error ( 'Could not get referrers between ' . date ( 'r', $startTimestamp ) . ' and ' . date ( 'r', $endTimestamp ) . '.' );
			$error->PdoErrorHelper ( $this->mDatabase->errorInfo (), __LINE__, __FILE__ );
			throw new Exception ( $error->GetErrorMsg () );
		}
	}
	...
	//! Used by the admin reports page - this gets those orders that are authorised.
	/*	Rationale - orders that are authorised should be in the packing queue -> those on
		onder should have had their dispatch date changed. This report should pick up orders
		that are neither (ie. have been missed/lost)*/
	/*!
	 ...
	 */
	function GetAuthorisedOrders($excludeNonSameDayDispatch = true, $startTimestamp = false, $endTimestamp = false) {
		// Set defaults
		if (! $startTimestamp) {
			$startTimestamp = mktime ( 0, 0, 0, 1, 1, 2008 );
		}
		...
		// Switch between on order/lost orders
		if ($excludeNonSameDayDispatch) {
			$dispatchDateSql = ' AND Dispatch_Date_ID = 1 ';
		} else {
			$dispatchDateSql = ' ';
		}
		...
		$sql = 'SELECT Order_ID
				FROM tblOrder
				WHERE Status_ID = 10 ' . $dispatchDateSql . '
					AND Created_Date BETWEEN ' . $startTimestamp . '
					AND ' . $endTimestamp . '
				ORDER BY Order_ID DESC';

		// Run the query
		if ($result = $this->mDatabase->query ( $sql )) {
			// Prep the results as an array of OrderModel objects
			while ( $resultObj = $result->fetch ( PDO::FETCH_OBJ ) ) {
