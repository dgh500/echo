...
/*		$headers  = 'MIME-Version: 1.0' . "\r\n";
	...
	function PrepareAndSendFailureEmail($address, $order, $prefix = 'ECHO') {$registry = Registry::getInstance ();
		...
		#if(mail($address,'Website Order - '.$prefix.$order->GetOrderId().' - FAILED',$body)) {
		if($this->mMail->Send()) {
			return true;
		} else {
			return false;
	...
	//! Gets all undispatched orders
	function GetUndispatchedOrders() {
		$sql = 'SELECT Order_ID FROM tblOrder WHERE Status_ID <> 6 ORDER BY Order_ID DESC';
		if ($result = $this->mDatabase->query ( $sql )) {
			$orders = $result->fetchAll ( PDO::FETCH_OBJ );
			// For each order, create a new instance of it and store it in the daysOrders variable
			foreach ( $orders as $order ) {
			...
			$error = new Error ( 'Could not get undispatched orders.' );
			$error->PdoErrorHelper ( $database->errorInfo (), __LINE__, __FILE__ );
			throw new Exception ( $error->GetErrorMsg () );
		}
	} // End GetUndispatchedOrders
	...
	function GetAuthorisedOrders($excludeNonSameDayDispatch = true, $startTimestamp = false, $endTimestamp = false) {
		...
		$sql = 'SELECT Order_ID
				FROM tblOrder
				WHERE Status_ID = 2 ' . $dispatchDateSql . '
	...
	function GetNotSentReviewEmail() {
		...
		AND Status_ID = 6
		AND Shipped_Date < '.$twoWeeksAgo.'
		ORDER BY Shipped_Date DESC'; #echo $sql;
		if ($result = $this->mDatabase->query ( $sql )) {
			while ( $resultObj = $result->fetch ( PDO::FETCH_OBJ ) ) {
				$tempOrder = new OrderModel ( $resultObj->Order_ID );
				if ($tempOrder->GetStatus ()->IsComplete ()) {
