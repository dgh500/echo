...
class PublicOrderView extends View {
...
EOT;
	...
	function LoadShippingAddress() {
		...
		$this->mPage .= '<br /><strong>Contact Details</strong><br />';
	...
	function LoadBasketDetails() {
		...
		$this->mPage .= <<<EOT
			...
			<br /><table id="publicOrderViewTable">
				<tr>
...
EOT;
		...
		foreach($this->mOrderItemController->GetProductsForOrder($this->mOrder) as $productItem) {
			...
			if($productItem->IsNonStockItem()) {
				$this->mPage .= '<tr><td><i><b>3-5 Day:</b> '.$productItem->GetDisplayName().'</i></td>';
			} else {
		...
		// Make upgrade obvious
		if($this->mOrder->GetTotalPostage() == 4.95) {
			$upgradePostageHeading = '(COURIER)';
		} else {
			$upgradePostageHeading = '';
		}

		// Display the bottom (totals) row of the table
		$this->mPage .= <<<EOT
				<tr>
					<td class="right"><strong>Sub-total:</strong><br /><strong>Delivery: {$upgradePostageHeading}</strong><br /><strong>Total:</strong>{$totalTakenHeading}</td>
