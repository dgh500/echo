...
class TagMenuView extends View {
...
EOT;
		...
		$allTags = $tagController->GetAllTagsFor ( $this->mCatalogue );
		$i = 1;
		foreach ( $allTags as $tag ) {
			$this->mPage .= <<<EOT
				d.add('{$i}',0,'{$tag->GetDisplayName()}','{$this->mAdminPath}/editArea/tag/{$this->mCatalogue->GetCatalogueId()}/{$tag->GetTagId()}/s','','editAreaContainer');
EOT;
			$i ++;
		}
	}
