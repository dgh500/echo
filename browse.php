...
$GLOBALS ['CONFIG'] ['IGNORE_DIRS'] = array ('./logs', './files', './images', './doc', './admin', './uploadImages', './visuals' );
...
if (! $GLOBALS ['CONFIG'] ['BG_IMAGE'])
	$GLOBALS ['CONFIG'] ['BG_IMAGE'] = $_SERVER ['SCRIPT_NAME'] . '?p=logo';
...
function iswebtype($file) {
	...
	if (in_array ( $ext, $GLOBALS ['CONFIG'] ['WEB_FILES'] ))
		return true;
	else
		return FALSE;
}
...
function isimgtype($file) {
	$ext = pathinfo ( $file );
	$ext = strtolower ( $ext ['extension'] );
	if (in_array ( $ext, $GLOBALS ['CONFIG'] ['IMG_FILES'] ))
		return true;
	else
		return FALSE;
}
...
function pagehead($stitle) {
	if ($stitle)
		$dtitle = $GLOBALS ['CONFIG'] ['PAGE_TITLE'] . " : " . $stitle;
	else
		$dtitle = $GLOBALS ['CONFIG'] ['PAGE_TITLE'];
	echo '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">' . '<html lang="en">' . '<head>' . '<title>' . $dtitle . '</title>' . '<meta http-equiv="Content-Type" content="text/html;charset=utf-8" >' . '<style type="text/css">' . 'td { font-size : 80%;font-family : tahoma;color: ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_1'] . ';}' . 'A:visited {color: ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_2'] . ';font-weight: bold;text-decoration: underline;}' . 'A:hover {color: ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_1'] . ';font-weight: bold;text-decoration: underline;}' . 'A:link {color: ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_2'] . ';font-weight: bold;text-decoration: underline;}' . 'A:active {color: #a6a6a6;font-weight: bold;text-decoration: underline;}' . 'textarea {border: 1px solid ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_2'] . ' ;color: black;background-color: white;}' . 'input.button{border: 1px solid ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_2'] . ';color: black;background-color: white;}' . 'input.text{border: 1px solid ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_2'] . ';color: black;background-color: white;}' . 'BODY {color: ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_1'] . '; FONT-SIZE: 10pt; FONT-FAMILY: arial,verdana,helvetica; scrollbar-base-color: ' . $GLOBALS ['CONFIG'] ['BG_COLOR_2'] . '; MARGIN: 0px 0px 10px; BACKGROUND-COLOR: ' . $GLOBALS ['CONFIG'] ['BG_COLOR_1'] . '}' . '.title {COLOR: ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_1'] . '; font: 13.5pt arial; FONT-WEIGHT: bold;}' . '.code {font-size: 10pt;}' . '.head {COLOR: ' . $GLOBALS ['CONFIG'] ['TXT_COLOR_1'] . '; font-size: 11pt; FONT-WEIGHT: bold;}' . '.copyright {FONT-SIZE: 8pt; FONT-FAMILY: arial,verdana,helvetica; COLOR: #FFFFFF}' . '.content {text-decoration: none; color: black; font: 10pt arial;}' . '</style>' . '</head><body>' . '<table cellpadding="0" cellspacing="0" align="center"><tr><td>' . '<table cellpadding="0" cellspacing="0" bgcolor="' . $GLOBALS ['CONFIG'] ['BG_COLOR_2'] . '"><tr><td>' . '<table width="100%" border="0" cellpadding="2" cellspacing="0">' . '<tr><td align="left" width="700" bgcolor="' . $GLOBALS ['CONFIG'] ['TB_COLOR_4'] . '"><img src="' . $GLOBALS ['CONFIG'] ['BG_IMAGE'] . '" alt="' . $pagetitle . '"> ' . $pagetitle . 

	'<tr><td>' . '<table cellpadding="0" cellspacing="0" width="100%"><tr><td width="100%">' . '<table width="100%">';

}

function pagefoot() {
	echo '</table></table></table></table>' . '<table width="100%"><tr><td align="right">' . '<font class="copyright">' . 'PHP Power Browse &copy; 2004-' . date ( 'Y' ) . ' <a href="http://arzdev.com">ArzDev</a>' . '</font></table></table>';
	die ();
}
...
function getmime($file) {
	...
	if (is_dir ( $file ))
		$type = 'folder';
	elseif (preg_match ( '/^application(.*?)compress|zip(.*?)$/', $mime ) || $ext == 'tar' || $ext == 'rar' || $ext == 'ace' || $ext == 'z' || $ext == 'gz')
		$type = 'archive';
	elseif ($mime == 'text/x-c' || $ext == 'c++' || $ext == 'c#')
		$type = 'c';
	elseif (($mime == 'application/octet-stream' || $ext == 'rpm' || $ext == 'exe' || $ext == 'bin') && (preg_match ( '/^(.*?)setup(.*?)$/', $file ) || preg_match ( '/^(.*?)install(.*?)$/', $file )))
		$type = 'setup';
	elseif ($mime == 'application/inf' || $ext == 'inf')
		$type = 'inf';
	elseif ($ext == 'php' || $ext == 'phtml' || $ext == 'php3' || $ext == 'php4' || $ext == 'php5' || preg_match ( '/^application(.*?)php(.*?)$/', $mime ))
		$type = 'php';
	elseif ($mime == 'text/html' || $ext == 'html' || $ext == 'htm')
		$type = 'html';
	elseif (preg_match ( '/^application(.*?)shockwave|flash(.*?)$/', $mime ) || $ext == 'swf' || $ext == 'fla')
		$type = 'swf';
	
	elseif ($mime == 'text/x-h')
		$type = 'h';
	elseif ($mime == 'application/msword')
		$type = 'msword';
	elseif ($mime == 'application/x-javascript')
		$type = 'javascript';
	elseif ($mime == 'application/xml' || $mime == 'text/xml')
		$type = 'xml';
	elseif ($mime == 'application/postscript')
		$type = 'postscript';
	elseif ($mime == 'application/pdf')
		$type = 'pdf';
	elseif ($mime == 'application/octet-stream')
		$type = 'exe';
	elseif ($mime == 'text/asp')
		$type = 'asp';
	...
	elseif ($ext == 'mk')
		$type = 'makefile';
	elseif (preg_match ( '/^application(.*?)powerpoint(.*?)$/', $mime ))
		$type = 'powerpoint';
	elseif (preg_match ( '/^application(.*?)cert(.*?)$/', $mime ))
		$type = 'crt';
	elseif (preg_match ( '/^(.*?)readme(.*?)$/', $file ))
		$type = 'readme';
	elseif (preg_match ( '/^application(.*?)java(.*?)$/', $mime ))
		$type = 'java';
	elseif (preg_match ( '/^application(.*?)excel$/', $mime ))
		$type = 'excel';
	elseif (preg_match ( '/^application(.*?)binary$/', $mime ))
		$type = 'binary';
