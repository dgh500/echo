...
class NewCustomerOrderHandler {
	...
	function Validate($postArr) {
		...
		foreach ( $postArr as $key => $value ) {
			switch ($key) {
				...
				case 'cardVerificationNumber' :
					...
					break;
				case 'vatFreeOrder':
						if($value === 1) { $this->mVatFreeOrder = true; } else { $this->mVatFreeOrder = false; }
					break;					
				default :
					if (strpos ( $key, 'productName' )) {
						...
						$this->mClean ['packages'] [] = new PackageModel ( $this->ExtractPackageId ( $key ) );
					...
					} elseif (strpos ( $key, 'packagePrice' )) {
						$this->SetPackagePrice ( $key, $value );
					} else {
		...
		if (isset ( $this->mClean ['packages'] )) {
			foreach ( $this->mClean ['packages'] as $package ) {
				...
				$toCount = $this->mClean ['packageQuantity'] [$package->GetPackageId ()];
				$toCount = intval ( $toCount ) + 1;
				for($packageCounter = 1; $packageCounter < $toCount; $packageCounter ++) {
					...
					foreach ( $contents as $product ) {
						...
						foreach ( $package->GetUpgradesFor ( $product ) as $upgrade ) {
							if (isset ( $postArr ['packageContentUgradeCheckbox' . $package->GetPackageId () . 'packageId' . $packageCounter . 'packageCounter' . $product->GetProductId () . 'productId' . $upgrade->GetProductId () . 'upgradeId'] )) {
								($this->mRegistry->debugMode ? fwrite ( $this->mFh, "Adding upgrade " . $upgrade->GetDisplayName () . " to basket line " . __LINE__ . ". \r\n" ) : NULL);
								$this->AddSkusToBasketForPackageProduct ( $package, $upgrade, $packageCounter );
								
								// The $upgrade that $product has been changed to
								$this->mClean ['packageUpgrade'] [$product->GetProductId ()] [] = $upgrade->GetProductId ();
								
								// No. times this upgrade has occurred
								if (isset ( $this->mClean ['upgradeCounter'] [$upgrade->GetProductId ()] )) {
									$this->mClean ['upgradeCounter'] [$upgrade->GetProductId ()] ++;
								} else {
									$this->mClean ['upgradeCounter'] [$upgrade->GetProductId ()] = 1;
								}
								// No. times this product has been upgraded
								if (isset ( $this->mClean ['packageUpgradeQty'] [$product->GetProductId ()] )) {
									$this->mClean ['packageUpgradeQty'] [$product->GetProductId ()] ++;
								} else {
									$this->mClean ['packageUpgradeQty'] [$product->GetProductId ()] = 1;
								}
								$upgradeLock = true;
							} // end if
						} // end foreach
	...
	//! Sets the price of the package whose ID is contained in $str as "packagePricePID"
	/*!
	 * @param $str [in] String - The "packagePricePID" string
	 * @param $quantity [in] Decimal - The price
	 * @return Void
	 */
	function SetPackagePrice($str, $price) {
		$priceArr = explode ( 'packagePrice', $str );
		$packageId = $priceArr [1];
		$this->mClean ['packagePrice'] [$packageId] = $price;
	}
	 ...
	 * 2) Adds the correct SKUs to the basket
	...
	function SaveOrder() {
		...
		$runningTotal = 0;
		// Tracks which products have already been upgraded (so only added as upgrades the correct number of times)
		$productAlreadyUpgraded = array ();
		
		$upgradeAlreadyChosen = array ();
		
		// Process packages
		if (isset ( $this->mClean ['packages'] )) {
			
			// Initialise $productAdreadyUpgraded Array for each product
			foreach ( $this->mClean ['packages'] as $package ) {
				foreach ( $package->GetContents () as $product ) {
					$productAlreadyUpgraded [$product->GetProductId ()] = 1;
					if (isset ( $this->mClean ['packageUpgrade'] [$product->GetProductId ()] )) {
						foreach ( $this->mClean ['packageUpgrade'] [$product->GetProductId ()] as $upgradeId ) {
							$upgradeAlreadyChosen [$upgradeId] = 1;
						}
					}
				}
			}
			
			// Actually process the package
			foreach ( $this->mClean ['packages'] as $package ) {
				// The correct number of times the package is in the basket
				for($i = 0; $i < $this->mClean ['packageQuantity'] [$package->GetPackageId ()]; $i ++) {
					...
					// Check for over-ruled price here
					if (isset ( $this->mClean ['packagePrice'] [$package->GetPackageId ()] )) {
						$this->mBasket->AddPackageToBasket ( $package, $this->mClean ['packagePrice'] [$package->GetPackageId ()] );
						$runningTotal += $this->mClean ['packagePrice'] [$package->GetPackageId ()];
					} else {
						$this->mBasket->AddPackageToBasket ( $package, $package->GetActualPrice () );
						$runningTotal += $package->GetActualPrice ();
					}
					
					// Remove the SKUs from the basketSkus array as you go, so only left with non-package ones to loop over
					$packageContents = $package->GetContents ();
					foreach ( $packageContents as $product ) {
						$foundSku = false;
						// If the product has been upgraded, then remove the upgrade, also make sure that it is only upgraded the X number of times it should be
						if (isset ( $this->mClean ['packageUpgrade'] [$product->GetProductId ()] ) && $productAlreadyUpgraded [$product->GetProductId ()] <= $this->mClean ['packageUpgradeQty'] [$product->GetProductId ()]) {
							foreach ( $this->mClean ['packageUpgrade'] [$product->GetProductId ()] as $upgradeId ) {
								if ($upgradeAlreadyChosen [$upgradeId] <= $this->mClean ['upgradeCounter'] [$upgradeId]) {
									$upgrade = new ProductModel ( $upgradeId );
									$allSkus = $upgrade->GetSkus ();
									foreach ( $allSkus as $sku ) {
										if (in_array ( $sku->GetSkuId (), $this->mClean ['basketSkus'] ) && ! $foundSku) {
											$position = array_search ( $sku->GetSkuId (), $this->mClean ['basketSkus'] );
											unset ( $this->mClean ['basketSkus'] [$position] );
											$upgradeAlreadyChosen [$upgradeId] ++;
											$this->mBasket->AddToBasket ( $sku, false, $package->GetUpgradePrice ( $product, $upgrade ), true ,false,$this->mVatFreeOrder);
											$runningTotal += $package->GetUpgradePrice ( $product, $upgrade );
											#echo 'Removed a SKU - '.$upgrade->GetDisplayName().'.<br />';
											$productAlreadyUpgraded [$product->GetProductId ()] ++;
											$foundSku = true;
										}
									}
								}
							...
							// No upgrade - Add to the basket with zero price, as a package but not a package upgrade
							$allSkus = $product->GetSkus ();
							foreach ( $allSkus as $sku ) {
								/*echo $sku->GetSkuId().'<br />';
								echo '<pre>';
								var_dump($this->mClean['basketSkus']);
								echo '</pre>';*/
								if (in_array ( $sku->GetSkuId (), $this->mClean ['basketSkus'] ) && ! $foundSku) {
									$position = array_search ( $sku->GetSkuId (), $this->mClean ['basketSkus'] );
									$this->mBasket->AddToBasket ( $sku, true, 0.0, false,$this->mVatFreeOrder );
									unset ( $this->mClean ['basketSkus'] [$position] );
									#echo 'Removed a SKU - '.$product->GetDisplayName().'.<br />';
									$foundSku = true;
								}
							}
						} // End if upgrade
						if (! $foundSku) {
							die ( 'Couldnt complete package on ' . $product->GetDisplayName () . ' ' );
						}
					} // End Foreach($packageContents as $product)
				} // End Looping over package QTY
			} // End looping over packages
		} // End if packages in basket
		

		// Process non-package items - these are what is left after packages have been processed
		if (isset ( $this->mClean ['basketSkus'] )) {
			...
			foreach ( $this->mClean ['basketSkus'] as $skuId ) {
				...
				$product = $sku->GetParentProduct ();
				// This adds to basket at the SKU price (unless overruled), but AddToBasket() takes into account multibuys
				// So to clear up: Multibuy > Staff Entered Price > Normal SKU Price is the precedance
				if (isset ( $this->mClean ['productPrice'] [$product->GetProductId ()] )) {
					$this->mBasket->AddToBasket ( $sku, false, $this->mClean ['productPrice'] [$product->GetProductId ()], false,$this->mVatFreeOrder );
				} else {
					$this->mBasket->AddToBasket ( $sku, false, $sku->GetSkuPrice (), false ,$this->mVatFreeOrder);
				}
			} // End foreach SKU
		} // End if non-packge items
		

		// Current SKUs in basket that arent upgrades
		$skus = $this->mBasket->GetSkus ( true );
		$skuTotal = 0;
		// Add the price to a sku running total
		foreach ( $skus as $sku ) {
			$skuTotal += $this->mBasket->GetOverruledSkuPrice ( $sku );
		}
		// Add sku total to total that includes packages and upgrades so far
		$runningTotal = $runningTotal + $skuTotal;
		
		// Create the order
		$order = $orderController->CreateOrder ( $this->mBasket );
		$order->SetGoogleCheckout(false);
		$order->SetCatalogue($order->GetBasket()->GetCatalogue());
		// Create the customer
		$customer = $customerController->CreateCustomer ();
		// Create the referrer
		$referrer = new ReferrerModel ( $this->mClean ['referrer'] );
		...
		$order->SetTotalPrice ( $this->mBasket->GetTotal () );
	...
	function LoadConfirmBar() {
		...
		$this->mPage .= '<input type="hidden" name="cardHoldersName" id="cardHoldersName" value="' . $this->mClean ['cardHoldersName'] . '" />';
